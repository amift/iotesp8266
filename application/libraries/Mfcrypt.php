 ...
 * @author      Miftahul a.k.a (amift) <miftahul81@gmail.com>
 */
class Mfcrypt{
    ...
    public function secure_it($string,$opt='e',$url_safe=true){
	 	if ($opt=='e') {
	        $enc_data = $this->enc($string);
	        if ($url_safe){
	            $enc_data = strtr($enc_data,array('+' => '.','=' => '-','/' => '~'));
	        }
	        $result=$enc_data;
	 	}else{
	        $dec_data = strtr($string,array('.' => '+','-' => '=','~' => '/'));
